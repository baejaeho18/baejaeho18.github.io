...
{% for pub in byyear %}
  <li class="pub">
    <p class="pub__title">
      ...
      {% if pub.type %}<span class="timeline__badge">{{ pub.type }}</span>{% endif %}
    </p>
    <p class="pub__meta">{{ pub.authors }} · <em>{{ pub.venue }}</em>, {{ pub.year }}{% if pub.award %} · <span class="pub__award">🏆 {{ pub.award }}</span>{% endif %}</p>
